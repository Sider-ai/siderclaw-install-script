...
Skills 是预构建的 JavaScript 函数库，按域名自动注入到 REPL 执行环境，挂载到 `siderRuntime.skills[<name>]` 命名空间下。**Skill 函数经过测试，比自己写 DOM 查询更可靠。**
...
2. **了解**：仔细阅读 `get` 返回的 `description` 和 `examples`，了解 **真实的访问键和函数签名**（注意：访问键不一定等于 skill 名称，例如 `google-sheets` skill 挂载在 `siderRuntime.skills.sheets` 而非 `siderRuntime.skills['google-sheets']`）
3. **使用**：在 REPL 中按 examples 所示的方式通过 `siderRuntime.skills.<key>.<fn>(...)` 调用
...
2. skills get name=google-sheets → 获取 description + examples
   → 了解到访问键是 siderRuntime.skills.sheets（不是 siderRuntime.skills.googleSheets！）
   → 了解到有 setCellValue/getRange/format 等函数
3. repl → siderRuntime.skills.sheets.setCellValue('A1', 'Hello')
```

### 内置 Skills

| Skill 名称 | 匹配域名 | siderRuntime.skills 访问键 |
|---|---|---|
| google | google.com | siderRuntime.skills.google |
| google-sheets | docs.google.com/spreadsheets | siderRuntime.skills.sheets |
| google-calendar-events | calendar.google.com | siderRuntime.skills.gcal |
| youtube | youtube.com | siderRuntime.skills.yt |
| linkedin-engagement | linkedin.com | siderRuntime.skills.linkedin |
| whatsapp | web.whatsapp.com | siderRuntime.skills.whatsapp |

## REPL 工具详解

### 核心用法
代码在页面上下文中执行，可直接访问 DOM、window 对象、页面变量。代码被包装在 async 函数中，支持 await，最后一个表达式的值作为返回值。统一通过 `window.siderRuntime`（或解构出的 `siderRuntime`）访问自动化能力，所有异步方法都返回 Promise。
...
// 使用 siderRuntime.fetch 做跨域请求（bypass 页面 CSP）
const res = await siderRuntime.fetch('https://api.example.com/data');
...
| 分类 | API | 说明 |
|---|---|---|
| Native 输入 | `siderRuntime.nativeClick(selector, { clickCount? })` | 真实鼠标点击（isTrusted: true），不被反爬机制识别 |
| Native 输入 | `siderRuntime.nativeType(selector, text, { replace? })` | 聚焦并输入文本（支持 IME） |
| Native 输入 | `siderRuntime.nativePressKey(key)` | 按下并释放一个键/组合键（如 'Enter'、'Ctrl+A'） |
| Native 输入 | `siderRuntime.nativeKeyDown(key)` / `nativeKeyUp(key)` | 分离的按下/释放，用于长按或手动组合 |
| Native 输入 | `siderRuntime.nativeHover(selector)` | 鼠标悬停 |
| Native 输入 | `siderRuntime.nativeDrag(fromSelector, toSelector)` | 拖拽 |
| Native 输入 | `siderRuntime.nativeScroll({ direction?, amount?, target? })` | 鼠标滚轮 |
| Native 输入 | `siderRuntime.nativeScrollTo(selectorOrPosition)` | scrollIntoView / 绝对滚动 |
| 网络 | `siderRuntime.fetch(url, init?)` | 与 window.fetch 接口兼容，但走 background 代理，跨域、绕过 CSP |
| 持久存储 | `siderRuntime.artifacts.list()` | 列出当前 session 命名空间的 artifacts |
| 持久存储 | `siderRuntime.artifacts.get(name)` | 读取 artifact |
| 持久存储 | `siderRuntime.artifacts.createOrUpdate(name, value, { mimeType? })` | 创建或更新 artifact（value 可以是字符串或任意 JSON 可序列化值） |
| 持久存储 | `siderRuntime.artifacts.delete(name)` | 删除 artifact |
| 标签页 | `siderRuntime.openInTab(url, { active?, setParent? })` | 在新标签页打开，返回 handle（含 `close()`、`closed`、`onclose`） |
| 等待 | `siderRuntime.waitFor(selector, timeout?)` / `waitForSelector(...)` | 等待元素出现 |
| 等待 | `siderRuntime.waitFor({ text, timeout? })` | 等待文本出现 |
| 主世界 | `siderRuntime.executeInMainWorld(iifeSource)` | 在页面主 world 执行一段 IIFE，返回可序列化值 |
| DOM 注入 | `siderRuntime.addElement(tag, attrs?)` / `addElement(parent, tag, attrs)` | 创建并插入元素 |
| DOM 注入 | `siderRuntime.addStyle(css)` | 插入 `<style>` |
| 音频 | `siderRuntime.audio.getState()` / `setMute({ isMuted })` | 当前 tab 的静音状态 |
| 日志 | `siderRuntime.readConsoleMessages({ onlyErrors?, pattern?, clear? })` | 读取 tab 的 console 消息历史 |
| 日志 | `siderRuntime.readNetworkRequests({ urlPattern?, clear? })` | 读取 tab 的网络请求历史 |
| 就绪 | `await siderRuntime.onReady` | DOMContentLoaded 完成 |
| Skills | `siderRuntime.skills[<key>]` | 已自动注入的 domain-matching skill 对象 |

### 原生输入事件函数

`siderRuntime.native*` 通过 Chrome DevTools Protocol 发送真实的浏览器事件（isTrusted: true），不会被反爬机制拦截。

⚠️ 原生输入是**备选方案**。优先使用标准 DOM 方法：
- 先尝试 `element.click()`、`element.focus()`、`element.value = text`
- 只有当标准 DOM 方法失败（页面检测/拦截合成事件）时，才使用 `siderRuntime.nativeClick` / `nativeType`

#### 原生输入示例

```javascript
// 简单点击和输入
await siderRuntime.nativeClick('button.start');
await siderRuntime.nativeType('input[name="username"]', 'john@example.com');
await siderRuntime.nativePressKey('Enter');

// 组合键（Ctrl+A 全选）
await siderRuntime.nativePressKey('Ctrl+a');

// 或手动按下/释放
await siderRuntime.nativeKeyDown('Control');
await siderRuntime.nativeKeyDown('a');
await siderRuntime.nativeKeyUp('a');
await siderRuntime.nativeKeyUp('Control');

// 悬停 + 拖拽 + 滚动
await siderRuntime.nativeHover('.menu-item');
await siderRuntime.nativeDrag('.item-source', '.drop-zone');
await siderRuntime.nativeScroll({ direction: 'down', amount: 600 });
await siderRuntime.nativeScrollTo('.section-footer');
```

### Artifact 存储 API

通过 `siderRuntime.artifacts` 将大量数据存入持久化 KV，避免在 Agent context 中传递。artifacts 自动以当前 sessionId 为命名空间隔离（conversationId）：

- `await siderRuntime.artifacts.createOrUpdate(name, value, { mimeType? })`
- `await siderRuntime.artifacts.get(name)`
- `await siderRuntime.artifacts.list()`
- `await siderRuntime.artifacts.delete(name)`
...
await siderRuntime.artifacts.createOrUpdate('products', products);
`products scraped: ${products.length}`

// 后续调用中读取并处理
const stored = await siderRuntime.artifacts.get('products');
const expensive = stored.filter(p => parseFloat(p.price?.replace('$','')) > 50);
await siderRuntime.artifacts.createOrUpdate('expensive-products', expensive);
...
1. repl 爬取数据 → `siderRuntime.artifacts.createOrUpdate('data', results)` → 只返回摘要
2. navigate 跳转下一页 → repl 继续爬取 → 追加到 artifact
3. 最终 repl 中 `siderRuntime.artifacts.get('data')` 处理全量数据 → 只返回最终结果

**Agent 直接管理 artifacts：**
使用 artifacts 工具的 create/read/list/delete 操作，无需写 REPL 代码即可存取数据。适合 Agent 写入分析总结、阶段性报告等。

**填写表单：**
用 repl 的 DOM 操作设置 value 并触发 input/change 事件。如果失败，改用 `siderRuntime.nativeType`。
...
mcporter call remote-browser.repl --args '{"code": "await siderRuntime.skills.google.getSearchResults()", "title": "Read Google results", "sessionId": "session_abc", "browserId": "<SenderId>"}' --output json
```
